...
export default function ExtendedPostCard(props: PostData) {
  ...
  const _handleDeletePost = async () => {
    if (props.postState == 2) {
      alert(
        "Cancela la oferta pendiente de este producto antes de eliminar la publicacion"
      );
      return;
    }
    if (props.postState == 4) {
      alert(
        "Cancela el intercambio pendiente de este producto antes de eliminar la publicacion"
      );
      return;
    }
    const formData = {
