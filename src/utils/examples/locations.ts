...
export const locations: ArgLocation[] = []

export const centers: Record<string, Center[]> = {}

raw.departamentos.map((departamento: Department) => {
  const location: ArgLocation = {
    label: departamento.provincia.nombre + ', ' + departamento.nombre,
    value: departamento.id
  }
  locations.push(location)
  const centersOfProvince: Center[] = []
  const _centers = new Set(departamento.id.split('').map((center: string) => center))
  _centers.forEach((center: string) => {
    centersOfProvince.push({
      label: center,
      value: center
    })
  })
  centersOfProvince.push()
  centers[departamento.id] = centersOfProvince
})
