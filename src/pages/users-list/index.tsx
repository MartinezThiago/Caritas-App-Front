...
export default function UsersSistemList({ user }: { user: User }) {
  ...
  const onPostSubmit = (formData: UserInfo) => {
    ...
    const newForm: any = newARRAYZ;
    console.log(newForm.mail);
    ...
    reader.onload = () => {
      ...
      // return
      console.log(`http://localhost:6389/CaritasBack/registrarUsuarioInterno`);
  ...
  const onPatchSubmit = (formData: UserInfo) => {
    ...
    newForm["id_usuario"] = (userRaw as any).find(
      (user: any) => user.mail === newForm.mail
    ).id;
    newForm["email"] = newForm.mail;
    newForm["nacimiento"] = new Date(newForm.nacimiento);
    delete newForm.mail;
    ...
    if (newForm["centro"] === undefined) {
      newForm["centro"] = (userRaw as any).find(
        (user: UserInfo) => user.mail === newForm.email
      ).centro;
    }
